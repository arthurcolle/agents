...
class RAGWithFlockMTL:
    ...
    def _setup_resources(self):
        ...
        try:
            ...
            self.conn.execute("""
                CREATE PROMPT('retrieval-prompt', 'Search for documents that are relevant to answering this question.');
            """)
            
            self.conn.execute("""
                CREATE PROMPT('generation-prompt', 'Based on the retrieved documents, answer the following question. If the documents do not contain relevant information, say so. Include citations to the document IDs you used in your answer.');
            """)
